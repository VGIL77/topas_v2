...
# HRM concepts to be implemented as bolt-on patterns (not external imports)
...
class TopasARC60M(nn.Module):
    ...
    def __init__(self, config: Optional[ModelConfig] = None):
        ...
        # HRM Hierarchical Reasoning Planner - CRITICAL INTEGRATION
        if not _HAS_HRM:
            raise RuntimeError("HRM integration failed - cannot initialize planner")
        
        hrm_cfg = dict(
            batch_size=1,
            seq_len=30*30,             # max ARC grid flattened
            vocab_size=10,             # colors
        ...
        print(f"[TOPAS] HRM planner initialized: {hrm_cfg['H_layers']}H+{hrm_cfg['L_layers']}L layers, halt_max={hrm_cfg['halt_max_steps']}")
    ...
    def grid_to_tokens(self, grid: torch.LongTensor) -> torch.LongTensor:
        """
        Convert ARC grid to token sequence for HRM planner
        
        Args:
            grid: Input grid [B, H, W]
            
        Returns:
            tokens: Flattened token sequence [B, H*W] with values clamped to [0, 9]
        """
    ...
    def forward(self, demos: List[Dict], test: Dict[str, torch.LongTensor],
        ...
        # STEP 0: HRM HIERARCHICAL REASONING PLANNER - CRITICAL INTEGRATION
        tokens = self.grid_to_tokens(test_grid)
        ...
        halted = (outputs["q_halt_logits"] > outputs["q_continue_logits"])
        if halted.any():
            eval_dsl_depth = min(eval_dsl_depth or self.config.max_dsl_depth, 3)
            eval_beam_width = min(eval_beam_width or self.config.max_beam_width, 8)
            if self.config.verbose:
                print(f"[HRM] Planner halted early - reducing search: depth={eval_dsl_depth}, beam={eval_beam_width}")
        elif self.config.verbose:
            print(f"[HRM] Planner continuing - full search depth maintained")
        ...
        if eval_use_dsl and (eval_dsl_depth or self.config.max_dsl_depth) > 0:
            ...
            # Normalize op_bias for safety
            op_bias = {k: float(v) for k, v in op_bias.items() if k in DSL_OPS}
            if not op_bias:
                op_bias = {}
            if self.relmem is not None:
            ...
            if dsl_pred is None:
                try:
                    ...
                    # Belt-and-suspenders op_bias normalization
                    from models.dsl_registry import DSL_OPS
                    op_bias = getattr(self, "relman", None).op_bias() if hasattr(self, "relman") else {}
                    op_bias = {k: float(v) for k, v in op_bias.items() if k in DSL_OPS}
                    if not op_bias:
                        op_bias = {}
        ...
        if False:  # Disabled
            ...
            try:
                ...
                from models.dsl_registry import DSL_OPS
                op_bias = self.relman.op_bias()
                # Normalize op_bias for safety - filter to valid registry ops only
                op_bias = {k: float(v) for k, v in op_bias.items() if k in DSL_OPS}
                if not op_bias:
                    op_bias = {}
                if op_bias:
                ...
                if candidate is not None:
                    ...
                    if extras["rule_vec"] is None:
                        op_bias = self.relman.op_bias()
                        # Normalize op_bias for safety
                        op_bias = {k: float(v) for k, v in op_bias.items() if isinstance(v, (int, float))}
                        if not op_bias:
                            op_bias = {}
                        if op_bias:
                            rule_vec = torch.zeros(32, device=brain.device)
